...
function UserTable() {
  ...
  const calculateAdditionalCost = (countdown, exitDate, entryDate) => {
    const overdueTime = (Date.now() - new Date(exitDate).getTime()) / 1000;
    if(Math.floor(calculateTimeDifference(entryDate, exitDate))<=countdown){
      return 0;
    }
    ...
    <div className="user-table">
      ...
      {data.map(res => (
        <div className="table-row" key={res.id}>
          ...
          <div className="table-cell">{res.cena + calculateAdditionalCost(res.countdown, res.exitDate, res.entryDate)}</div>
