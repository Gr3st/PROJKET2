...
export function useAdminService() {
  ...
  const generateCSV = (data) => {
    ...
    const rows = data.map(user => [
      ...
      formatCell(user.id || ''),
      formatCell(Math.floor((new Date(user.exitDate).getTime() - new Date(user.entryDate).getTime()) / 1000)>=user.countdown?Math.ceil((((new Date(user.exitDate).getTime() - new Date(user.entryDate).getTime()) / 1000)/60)*1.5):`${user.cena}zł`),
